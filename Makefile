...
NN_LIBS=-llept -std=c++0x -lpthread
...
NN_LEARN_FLAGS=-DNUM_CPU=2 -DNOT_VERBOSE_DEBUG=1 -O3 -funroll-loops -mmmx -msse -msse2 -g#-pg
...
nn_compare: ${NN_COMPARE_SOURCES} ${NN_HEADERS}
	g++ ${NN_COMPARE_FLAGS} -o nn_compare ${NN_COMPARE_SOURCES} ${NN_LIBS}

nn_learn: ${NN_LEARN_SOURCES} ${NN_HEADERS}
	g++ ${NN_LEARN_FLAGS} -o nn_learn ${NN_LEARN_SOURCES} ${NN_LIBS}

get_theta: ${NN_HEADERS} ${NN_SOURCES} nn_get_theta.cpp
	g++ ${NN_COMPARE_FLAGS} -o nn_get_theta ${NN_SOURCES} nn_get_theta.cpp -g ${NN_LIBS}
